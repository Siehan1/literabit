        ...
        <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
                ...
                <div class="flex flex-col items-center">
                    <form action="{{route('level.update', $levelTreshodls->id)}}" method="POST" class="space-y-6 w-2xl">
                        @csrf
                        <div class="flex gap-2 flex-col">
                            <label for="level" class="block text-sm font-medium text-teks">Level<span class="text-red-500">*</span></label>
                            <input value="{{ $levelTreshodls->level }}" type="number" name="level" class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring focus:border-primary focus:ring-primary/20 transition-all duration-200">
                        </div>
                        <div class="flex flex-col gap-2">
                            <label for="" class="block text-sm font-medium text-teks">Experience<span class="text-red-500">*</span></label>
                            <input value="{{ $levelTreshodls->required_xp }}" type="number" name="required_xp" class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring focus:border-primary focus:ring-primary/20 transition-all duration-200">
                        </div>
