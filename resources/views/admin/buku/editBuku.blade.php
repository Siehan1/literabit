<x-dashboardComponent.admin>
    ...
    <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
            <h2 class="text-2xl font-bold text-primary mb-8 text-center">Edit Buku {{$buku->judul}}</h2>
            ...
            @endif
            <form action="{{ route('buku.update', $buku->slug) }}" method="POST" class="space-y-6">
                @csrf
                @method('POST')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select id="genre" required
                            class="w-full px-4 py-2.5 rounded-xl border-2 border-gray-200 focus:border-primary focus:ring focus:ring-primary/20 focus:outline-none transition-all duration-200"
                            name="genre">
                            <option value="" disabled {{ is_null($buku->genre_id) ? 'selected' : '' }}>Pilih genre buku</option>
                            @foreach ($genres as $genre)
                                <option value="{{ $genre->id }}" {{ $buku->genre_id == $genre->id ? 'selected' : '' }}>
                                    {{ $genre->nama_genre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" id="level" required 
                            class="w-full px-4 py-2.5 rounded-xl border-2 border-gray-200 focus:border-primary focus:ring focus:ring-primary/20 focus:outline-none transition-all duration-200" 
                            value="{{ $buku->level_required }}"
                            name="level" min="0">
                    </div>

                    <!-- Sinopsis -->
                    <div class="space-y-2 md:col-span-2">
                        <label for="sinopsis" class="block text-sm font-medium text-teks">Sinopsis</label>
                        <textarea id="sinopsis" required
                            class="w-full px-4 py-2.5 rounded-xl border-2 border-gray-200 focus:border-primary focus:ring focus:ring-primary/20 focus:outline-none transition-all duration-200"
                            name="sinopsis" placeholder="Sinopsis"
                            rows="4">{{ $buku->sinopsis }}</textarea>
                    </div>
